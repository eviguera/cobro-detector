...
async function main() {
  ...
  const exists = fs.existsSync(path.join(process.cwd(), dbTypesPath))
  check(exists, 'database.types.ts existe')
  ...
  const requiredDeps = ['next', 'supabase', 'mercadopago', 'docx', 'puppeteer', 'zod', 'google/generative-ai']
  for (const dep of requiredDeps) {
    const inDeps = packageJson.dependencies?.[dep] || packageJson.devDependencies?.[dep]
    check(!!inDeps, `Dependencia instalada: ${dep}`)
  }
  ...
  // Resumen
  console.log('\n' + '='.repeat(50))
  console.log(`📊 RESUMEN: ${passed} pasaron, ${failed} fallaron, ${warnings.length} advertencias`)
