...
import { formatCLP, getAnomalyTypeLabel, getSeverityLabel } from '@/lib/utils'
import { escapeXml } from '@/lib/security'
import { SEVERITY } from '@/lib/constants'
import type { Analysis, DetectedAnomaly, ParsedTransaction } from '@/domain'
...
export default function DownloadReportButton({ analysis, anomalies, transactions }: Props) {
  ...
  const generateHTML = () => {
    ...
    const date = new Date(analysis.created_at).toLocaleDateString('es-CL')
    ...
    const cobroCards = anomalies.map((a, i) => {
      ...
      return `<div class="cobro" style="border:1px solid #e5e7eb;border-left:5px solid;${SEVERITY[a.severity]?.borderStyle ?? 'border-left-color:#6b7280;'}border-radius:10px;padding:20px 22px;margin-bottom:16px;">
        ...
        <p style="font-size:14px;font-weight:600;color:#111827;margin-bottom:4px;">${escapeXml(a.title)}</p>
        <p style="font-size:12px;color:#6b7280;margin-bottom:4px;">${escapeXml(a.description)}</p>
        ${a.detail ? `<p style="font-size:11px;color:#6b7280;margin-bottom:8px;padding:8px 10px;background:#f9fafb;border-radius:6px;font-family:monospace;">${escapeXml(a.detail)}</p>` : ''}
...
<head>
<meta charset="UTF-8">
<title>Reporte CobroDetector · ${escapeXml(analysis.file_name)}</title>
<style>
...
<div class="header">
  ...
  <div class="subtitle">
    <span>Archivo: <strong>${escapeXml(analysis.file_name)}</strong></span>
...
<div class="ai-summary">
  ...
  <p>${escapeXml(analysis.ai_summary)}</p>
</div>` : ''}
