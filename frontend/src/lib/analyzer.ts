...
import { detectBank } from './parser'

const severityScore: Record<string, number> = { high: 3, medium: 2, low: 1 }

export function deduplicateAnomalies(anomalies: DetectedAnomaly[]): DetectedAnomaly[] {
  if (anomalies.length <= 1) return anomalies

  const groups: DetectedAnomaly[][] = []

  for (const anomaly of anomalies) {
    let matched = false
    for (const group of groups) {
      const groupRefs = new Set(group.flatMap((a) => a.transactionRefs))
      const candidateRefs = new Set(anomaly.transactionRefs)
      const hasOverlap = [...candidateRefs].some((r) => groupRefs.has(r))
  ...
  for (const group of groups) {
    ...
    const ai = group.filter((a) =>
      a.description?.length > 80 && !a.title?.match(/COBRO_DOBLE|COBRO_ALTO|COBRO_INCORRECTO/i)
    )
    const csvLabeled = group.filter((a) =>
      a.title?.match(/COBRO_DOBLE|COBRO_ALTO|COBRO_INCORRECTO/i)
    )
    const rules = group.filter((a) =>
      a.description?.length <= 80 && !a.title?.match(/COBRO_DOBLE|COBRO_ALTO|COBRO_INCORRECTO/i)
    )

    const best = group.reduce((a, b) =>
      (severityScore[a.severity] || 0) > (severityScore[b.severity] || 0) ? a : b
    )

    const severity = best.severity
    const type = group[0].type

    const title = (csvLabeled[0]?.title || ai[0]?.title || rules[0]?.title || group[0].title || '')

    const description = (ai[0]?.description || csvLabeled[0]?.description || rules[0]?.description || group[0].description || '')
    ...
    const recoverableAmount = Math.max(...group.map((a) => a.recoverableAmount))

    const transactionRefs = [...new Set(group.flatMap((a) => a.transactionRefs))]
...
export function detectAnomaliesRules(transactions: ParsedTransaction[]): DetectedAnomaly[] {
  ...
  Object.entries(saleGroups).forEach(([_key, sales]: [string, ParsedTransaction[]]) => {
    ...
    const relatedCommissions = sortedTxs.filter((tx) =>
    ...
    if (relatedCommissions.length > 1) {
      ...
      const amounts = relatedCommissions.map((c) => Math.abs(c.amount))
      ...
      if (isSameCommission) {
        const totalExtra = relatedCommissions.slice(1).reduce((sum: number, tx) => sum + Math.abs(tx.amount), 0)
        anomalies.push({
          type: 'duplicate_commission',
          severity: 'high',
          ...
          transactionRefs: relatedCommissions.slice(1).map((t) => t.id),
        })
      }
    }
  })
  ...
  sortedTxs2.forEach((tx, i) => {
    if (checked.has(tx.id)) return
    const txDate = new Date(tx.date)
    const duplicates = sortedTxs2.filter((other, j) => {
    ...
    if (duplicates.length > 0) {
      ...
      const totalDup = duplicates.reduce((sum: number, d) => sum + Math.abs(d.amount), 0)
      anomalies.push({
        type: 'duplicate_commission',
        severity: 'high',
...
export async function analyzeFile(
  filePath: string,
  fileType: string,
  _options?: { userId?: string; companyId?: string; fileName?: string }
  ...
  try {
    ...
    const allAnomalies = deduplicateAnomalies([
      ...ruleAnomalies,
      ...labeledAnomalies,
      ...aiResult.anomalies,
    ])
