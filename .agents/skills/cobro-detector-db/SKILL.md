...
- **RLS**: Habilitado en 11 de 12 tablas (`api_logs` es la excepción)
...
1. **`handle_new_user` en `public` con SECURITY DEFINER** — debería moverse a schema `internal`.
2. **`api_logs` sin RLS** — única tabla sin protección a nivel de fila.
3. **`orders` sin `updated_at` ni trigger** — no se registra cuándo se actualiza una orden.
4. **`analyses` sin `updated_at`** — no hay timestamp de última modificación.
5. **`search_path` no configurado** en funciones SECURITY DEFINER — el linter de Supabase lo reporta como warning.
